...
const programContent = {
  lawn: {
    alpha: {
      ...
      features: [
        ...
        'Front + back coverage with tidy cleanup',
        'Owner check-ins and kickoff edging refresh',
      ],
      pills: [],
      details: [
        {
    ...
    beta: {
      ...
      features: [
        'Bi-weekly mowing with trims and edges',
        'Front + back yard coverage for curb appeal',
        'Flexible schedule around weather swings',
      ],
    ...
    delta: {
      ...
      features: [
        'Front-yard mowing and trim essentials',
        'Edge refresh for sidewalks and driveway lips',
        'Budget-friendly visits with optional backyard add',
      ],
  ...
  snow: {
    alpha: {
      ...
      features: [
        'Automatic priority dispatch every snowfall',
        'Driveway, walkways, and entry steps cleared',
        'Optional de-icing and photo updates',
      ],
    ...
    beta: {
      ...
      features: [
        'Clearing every snowfall on standard route',
        'Driveway and walkway coverage with shovel finish',
        'Add de-icing per visit as needed',
      ],
    ...
    delta: {
      ...
      features: [
        'As-available pushes when you need a clear path',
        'Driveway-first service with walkway optional',
        'Budget-minded per-visit pricing',
      ],
  ...
  landscaping: {
    alpha: {
      ...
      features: [
        'Design consultations with plans and material list',
        'Stone, bed, lighting, and planting installation',
        'Post-build cleanup with optional follow-up visit',
      ],
    ...
    beta: {
      ...
      features: [
        'Bed reshaping with crisp edging',
        'Mulch or rock upgrades plus selective planting',
        'Cleanup and watering guidance after install',
      ],
    ...
    delta: {
      ...
      features: [
        'Quick cleanups and light bed maintenance',
        'Edge refresh with spot planting optional',
        'Debris removal and simple aftercare guidance',
      ],
  ...
  cleanup: {
    alpha: {
      ...
      features: [
        'Full-property sweep with bagging and haul-away',
        'Bed detailing, edging refresh, and pathway sweeps',
        'Light pruning options with seasonal fertilizer touch-up',
      ],
    ...
    beta: {
      ...
      features: [
        'Leaf and stick removal with blow-and-bag',
        'Beds and edges tidied for a seasonal reset',
        'Haul-away included plus optional pruning or fertilizer',
      ],
    ...
    delta: {
      ...
      features: [
        'Front-yard sweep and bag for curb pickup',
        'Walkways and entry tidied with light edging',
        'Optional backyard add or haul-away service',
      ],
...
programSections.forEach((section) => {
  ...
  const featureTitle = featureBox?.querySelector('.program-feature-title');
  const featurePrice = featureBox?.querySelector('.program-feature-price');
  const featureList = featureBox?.querySelector('.program-feature-list');
  const selectButton = featureBox?.querySelector('.program-select-btn');

  let activeTierKey = 'alpha';

  const prefillForm = (tier, scrollToQuote = false) => {
    ...
    if (detailsField) {
      const featureSummary = (tier.features || []).join('; ');
      const composed = [`Selected package: ${tier.label}.`, tier.description, featureSummary ? `Includes: ${featureSummary}.` : '']
        .filter(Boolean)
        .join(' ');
      detailsField.value = composed.trim();
    }
  ...
  const updateFeatureBox = (tier) => {
    if (!featureBox) return;
    if (featureTitle) featureTitle.textContent = tier.label;
    if (featurePrice) featurePrice.textContent = [tier.price, tier.term].filter(Boolean).join(' ');
    if (featureList) {
  ...
  const renderTier = (tierKey, scrollToQuote = false) => {
    ...
    updateFeatureBox(tier);
    prefillForm(tier, scrollToQuote);
  };

  programCards.forEach((card) => {
    card.addEventListener('click', () => renderTier(card.dataset.program || 'alpha', true));
  });

  if (selectButton) {
    selectButton.addEventListener('click', () => {
      const chosenTier = serviceContent[activeTierKey];
      if (chosenTier) {
        prefillForm(chosenTier, true);
      }
    });
  }
