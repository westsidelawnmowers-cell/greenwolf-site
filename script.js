...
// Hide header on swipe/scroll for mobile comfort
...
if (header) {
  window.addEventListener(
    ...
    () => {
      ...
      const scrollingDown = currentY > lastScrollY + 12;
      const shouldHide = scrollingDown && currentY > 140;
...
const selectedFeaturesByService = {};
...
programSections.forEach((section) => {
  ...
  const summaryBox = document.querySelector(`[data-service-summary="${service}"]`);
  const summaryList = summaryBox?.querySelector('.summary-list');
  const addToQuoteBtn = summaryBox?.querySelector('.add-to-quote');
  const clearBtn = summaryBox?.querySelector('[data-clear]');
  const selectionSet = (selectedFeaturesByService[service] =
    selectedFeaturesByService[service] || new Set());

  const getFeatureLabel = (card) =>
    card?.querySelector('.option-title, h3')?.textContent?.trim() || '';

  const refreshSelectionStyles = () => {
    detailCards.forEach((card) => {
      const label = getFeatureLabel(card);
      card.classList.toggle('is-selected', selectionSet.has(label));
    });
  ...
  const updateSummaryUI = () => {
    ...
    if (!selectionSet.size) {
      ...
      placeholder.textContent = 'Tap a feature to add it here.';
      summaryList.appendChild(placeholder);
      return;
    }
    Array.from(selectionSet).forEach((item) => {
  ...
  if (addToQuoteBtn) {
    addToQuoteBtn.addEventListener('click', () => {
      ...
      const quoteDetails = document.querySelector('.quote-form textarea[name="details"]');
      const selectionText = `${service} features: ${Array.from(selectionSet).join(', ')}`;
      if (quoteDetails) {
        const existing = quoteDetails.value.trim();
        quoteDetails.value = existing ? `${existing}\n${selectionText}` : selectionText;
        setStatus('Added your selected features to the quote.', 'info');
      }
    });
  }
  ...
  [...detailCards, ...optionCards].forEach((card) => {
    card.addEventListener('click', () => {
      toggleFeature(getFeatureLabel(card));
    });
  });
  ...
  programCards.forEach((card) => {
    card.addEventListener('click', () => {
      ...
      const focusTarget = section.querySelector('.detail-board') || section;
