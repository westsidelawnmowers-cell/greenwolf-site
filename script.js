...
const programContent = {
  lawn: {
    alpha: {
      ...
      features: ['Weekly mowing + trimming', 'Crisp edging and cleanup', 'Owner check-ins + fertilizer'],
    ...
    beta: {
      ...
      features: ['Bi-weekly mowing', 'Front + back attention', 'Edge + blow cleanup'],
      tag: 'Beta • Balanced',
    ...
    delta: {
      ...
      features: ['Front-yard focus', 'Edge + trim basics', 'Optional backyard add-on'],
      tag: 'Delta • Value',
  ...
  snow: {
    alpha: {
      ...
      features: ['Priority route', 'Driveway + walkway clears', 'Optional de-icing'],
      tag: 'Alpha • Top tier',
    ...
    beta: {
      ...
      features: ['Every snowfall service', 'Driveway + walkways', 'Shovel finish on steps'],
      tag: 'Beta • Balanced',
    ...
    delta: {
      ...
      features: ['Driveway push focus', 'As-available timing', 'Walkway add-on optional'],
      tag: 'Delta • Value',
  ...
  landscaping: {
    alpha: {
      ...
      features: ['Design consult + plan', 'Premium installs', 'Lighting + cleanup'],
      tag: 'Alpha • Top tier',
    ...
    beta: {
      ...
      features: ['Bed reshaping', 'Rock or mulch upgrade', 'Selective planting'],
      tag: 'Beta • Balanced',
    ...
    delta: {
      ...
      features: ['Weeding + tidy', 'Edge refresh', 'Light material swaps'],
      tag: 'Delta • Value',
  ...
  cleanup: {
    alpha: {
      ...
      features: ['Full debris removal', 'Bed + edge detailing', 'Haul-away included'],
      tag: 'Alpha • Top tier',
    ...
    beta: {
      ...
      features: ['Leaf + stick clear', 'Front + back sweep', 'Bagged and ready for pickup'],
      tag: 'Beta • Balanced',
    ...
    delta: {
      ...
      features: ['Front-yard focus', 'Leaf collection', 'Bagged for pickup'],
      tag: 'Delta • Value',
...
const fillFormDetails = (service, tierKey) => {
  const form = document.querySelector(`form[data-service-form="${service}"]`);
  const tier = programContent[service]?.[tierKey];
  if (!form || !tier) return;

  const detailsField = form.querySelector('textarea[name="details"]');
  if (detailsField) {
    const featureLine = tier.features?.length
      ? `Includes: ${tier.features.join(', ')}`
      : tier.description || '';
    detailsField.value = `${tier.label} selected. ${featureLine}`.trim();
  }
};
...
const scrollToQuoteForm = () => {
  ...
  if (quoteSection) {
    ...
    setTimeout(() => {
      const firstInput = quoteSection.querySelector('form input, form textarea, form select');
...
programSections.forEach((section) => {
  ...
  const featureBoxes = Array.from(section.querySelectorAll('.program-feature-box'));

  featureBoxes.forEach((box) => {
    const boxKey = box.dataset.program;
    const tier = serviceContent[boxKey];
    const list = box.querySelector('.program-feature-list');
    const button = box.querySelector('.select-program');
    if (!tier) return;

    if (list) {
      list.innerHTML = '';
      (tier.features || tier.pills || []).slice(0, 4).forEach((feature) => {
    ...
    if (button) {
      button.textContent = `Select ${tier.label}`;
    }
  });

  const renderTier = (tierKey, { prefillForm = false, scrollForm = false } = {}) => {
    ...
    featureBoxes.forEach((box) => {
      const isActive = box.dataset.program === tierKey;
      box.classList.toggle('is-active', isActive);
    });

    syncFormTier(service, tier.label);
    if (prefillForm) {
      fillFormDetails(service, tierKey);
    }
    if (scrollForm) {
      scrollToQuoteForm();
    }
  ...
  programCards.forEach((card) => {
    card.addEventListener('click', () => renderTier(card.dataset.program || 'alpha', { prefillForm: true, scrollForm: true }));
  });

  const selectButtons = Array.from(section.querySelectorAll('.select-program'));
  selectButtons.forEach((btn) => {
    btn.addEventListener('click', () => {
      renderTier(btn.dataset.program || 'alpha', { prefillForm: true, scrollForm: true });
    });
  });
