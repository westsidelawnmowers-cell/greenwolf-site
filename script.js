...
programSections.forEach((section) => {
  ...
  if (programBody) {
    programBody.setAttribute('aria-hidden', 'true');
  }
  ...
  const openProgramBody = () => {
    section.classList.add('program-open');
    if (programBody) {
      programBody.setAttribute('aria-hidden', 'false');
    }
  };
  ...
  programCards.forEach((card) => {
    card.addEventListener('click', () => {
      ...
      openProgramBody();
      const target = programBody || section.querySelector('.program-main') || section;
