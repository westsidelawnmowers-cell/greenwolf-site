...
// Mobile nav toggle and header hide on swipe/scroll
...
let touchStartY = 0;
const toggleHeaderHidden = (hidden) => siteHeader?.classList.toggle('is-hidden', hidden);

window.addEventListener(
  'scroll',
  () => {
    const current = window.scrollY;
    const hide = current > lastScrollY + 8 && current > 60;
    const show = current < lastScrollY - 6;
    if (hide) toggleHeaderHidden(true);
    if (show) toggleHeaderHidden(false);
    lastScrollY = current;
  },
  { passive: true }
);
...
window.addEventListener(
  ...
  (event) => {
    ...
    if (deltaY < -18) toggleHeaderHidden(true);
    if (deltaY > 12) toggleHeaderHidden(false);
  },
  { passive: true }
);
...
programSections.forEach((section) => {
  ...
  const serviceForm = document.querySelector(`form[data-service-form="${service}"]`);
  const selectionList = serviceForm?.querySelector('.selected-features-list');
  const selectionInput = serviceForm?.querySelector('input[name="selected_features"]');
  ...
  const refreshSelections = () => {
    ...
    if (selectionInput) {
      selectionInput.value = Array.from(selectedFeatures).join(', ');
    }
  };
  ...
  const clearSelections = () => {
    ...
    optionCards.forEach((card) => {
      const checkbox = card.querySelector('.option-check');
      if (checkbox) checkbox.checked = false;
    });
    refreshSelections();
  };

  const toggleSelection = (card, checked) => {
    const title = card.querySelector('.option-title')?.textContent?.trim();
    if (!title) return;
    if (checked) {
      selectedFeatures.add(title);
    } else {
      selectedFeatures.delete(title);
    }
    refreshSelections();
  };
  ...
  optionCards.forEach((card) => {
    ...
    checkbox.addEventListener('change', (event) => {
      toggleSelection(card, event.target.checked);
    });
  ...
  programCards.forEach((card) => {
    card.addEventListener('click', () => {
      ...
      const body = section.querySelector('.program-body');
      if (body && window.innerWidth < 960) {
