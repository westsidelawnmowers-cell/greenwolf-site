...
const packageFeatures = {
  lawn: {
    alpha: {
      label: 'Alpha Turf Program',
      features: ['Weekly mowing with trimming and edging', 'Owner check-ins to keep quality high', 'Walkways blown clear every visit'],
    },
    beta: {
      label: 'Beta Turf Program',
      features: ['Bi-weekly mowing and trimming', 'Crisp edging on sidewalks and drives', 'Flexible scheduling around growth'],
    },
    delta: {
      label: 'Delta Turf Program',
      features: ['Front-yard mowing focus', 'Quick trims on key edges', 'Budget-friendly upkeep'],
    },
  },
  snow: {
    alpha: {
      label: 'Alpha Snow Program',
      features: ['Priority route every snowfall', 'Walks, driveway, and steps cleared', 'First-wave response with de-ice option'],
    },
    beta: {
      label: 'Beta Snow Program',
      features: ['Standard route every snowfall', 'Driveway and walkway clearing', 'Reliable timing with clear communication'],
    },
    delta: {
      label: 'Delta Snow Program',
      features: ['As-available clearing', 'Driveway-first focus', 'Budget-friendly visits when you need them'],
    },
  },
  landscaping: {
    alpha: {
      label: 'Alpha Landscape Program',
      features: ['Design, install, and premium builds', 'Bed reshapes and high-end materials', 'Project management from plan to finish'],
    },
    beta: {
      label: 'Beta Landscape Program',
      features: ['Targeted refresh projects', 'Smart upgrades with balanced budgets', 'Planting, mulch, and edging updates'],
    },
    delta: {
      label: 'Delta Landscape Program',
      features: ['Essential upkeep and touch-ups', 'Weeding, edging, and light planting', 'Quick refresh to maintain curb appeal'],
    },
  },
  cleanup: {
    alpha: {
      label: 'Alpha Cleanup Program',
      features: ['Full property leaf and debris removal', 'Bed detailing and edging refresh', 'Haul-away included with premium pruning'],
    },
    beta: {
      label: 'Beta Cleanup Program',
      features: ['Leaf and debris clear-out', 'Bed touch-ups and sweeping', 'Haul-away included with flexible add-ons'],
    },
    delta: {
      label: 'Delta Cleanup Program',
      features: ['Front-yard focused tidy', 'Bagged for pickup if preferred', 'Quick refresh for curb-facing areas'],
    },
  },
};
...
const goToForm = (service, tierKey) => {
  const form = document.querySelector(`form[data-service-form="${service}"]`);
  if (!form) return;

  const tierData = programContent[service]?.[tierKey];
  const featureData = packageFeatures[service]?.[tierKey];
  const label = featureData?.label || tierData?.label || '';
  ...
  if (details && featureData?.features?.length) {
    details.value = `${label} includes:\n- ${featureData.features.join('\n- ')}`;
  }
...
programSections.forEach((section) => {
  ...
  const featurePanel = section.querySelector('[data-feature-panel]');
  const featureList = section.querySelector('[data-feature-list]');
  const featureTitle = section.querySelector('.feature-title');
  const selectButton = section.querySelector('[data-select-package]');
  let currentTier = 'alpha';
  ...
  const renderTier = (tierKey) => {
    ...
    const featureData = packageFeatures[service]?.[tierKey];
    if (featurePanel && featureList && featureData) {
      featureTitle.textContent = `${featureData.label} includes`;
      ...
      if (selectButton) {
        selectButton.textContent = `Select ${featureData.label}`;
        selectButton.setAttribute('aria-label', `Select ${featureData.label}`);
      }
    }
  };
  ...
  if (selectButton) {
    selectButton.addEventListener('click', () => goToForm(service, currentTier));
  }
