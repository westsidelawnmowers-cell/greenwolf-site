...
programSections.forEach((section) => {
  ...
  const detailStack = section.querySelector('.detail-stack');
  const lineStack = section.querySelector('.selected-lines');
  const priceValueEls = section.querySelectorAll('.program-selected .price-value');
  const priceTermEls = section.querySelectorAll('.program-selected .price-term');
  const priceFrequencyEls = section.querySelectorAll('.program-selected .price-frequency');
  const summaryBonusEls = section.querySelectorAll('.program-selected .program-bonus');
  ...
  const renderTier = (tierKey) => {
    ...
    if (lineStack) {
      ...
      const buildLine = (line) => {
        const row = document.createElement('div');
        row.className = 'selected-line';

        const info = document.createElement('div');
        const title = document.createElement('p');
        title.className = 'line-title';
        title.textContent = line.label || line.title || '';
        info.appendChild(title);

        const note = line.note || line.description;
        if (note) {
          const noteEl = document.createElement('p');
          noteEl.className = 'line-note';
        ...
        if (line.price) {
          ...
          priceEl.textContent = line.price;
      ...
      const primaryOption = tier.options?.[0];
      if (primaryOption?.suboptions?.length) {
        primaryOption.suboptions.forEach((sub) => buildLine(sub));
      } else if (tier.options?.length) {
        tier.options.slice(0, 3).forEach((option) => buildLine(option));
      }
    }
