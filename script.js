...
const LAWN_PLAN_LABELS = Object.freeze({
...
function setupLawnQuoteForm() {
  ...
  const quoteSection = document.getElementById('quote');
  const planSelect = form.querySelector('#planCode');
  const status = document.getElementById('lawn-quote-status');
  const selectedPlanName = document.getElementById('selected-plan-name');
  const selectedPlanCopy = document.getElementById('selected-plan-copy');
  ...
  const planCards = Array.from(document.querySelectorAll('.plan-showcase .package-card[data-plan-code]'));
  const defaultCity = form.elements.city?.value || '';
  const defaultProvince = form.elements.province?.value || '';
  ...
  const updateSelectedPlan = (planCode) => {
    const normalizedPlan = (planCode || '').toLowerCase();
    const planLabel = LAWN_PLAN_LABELS[normalizedPlan] || 'Not selected yet';

    if (planSelect && planSelect.value !== normalizedPlan) {
      planSelect.value = normalizedPlan;
    }

    if (selectedPlanName) {
      selectedPlanName.textContent = planLabel;
    }

    if (selectedPlanBanner) {
      selectedPlanBanner.classList.toggle('is-empty', !normalizedPlan);
    }

    if (selectedPlanCopy) {
      selectedPlanCopy.textContent = normalizedPlan
        ? `${planLabel} selected. Fill out your contact details below and we will build the quote around that plan.`
        : 'Choose Alpha, Beta, or Delta above and the form will lock that plan in automatically.';
    }

    planCards.forEach((card) => {
      const isSelected = card.dataset.planCode === normalizedPlan;
      card.classList.toggle('is-selected', isSelected);
      card.querySelector('.plan-select-button')?.classList.toggle('is-selected', isSelected);
    });
  };

  planCards.forEach((card) => {
    const button = card.querySelector('.plan-select-button');
    if (!button) return;

    button.addEventListener('click', (event) => {
      event.preventDefault();
      updateSelectedPlan(button.dataset.planCode || card.dataset.planCode || '');
  ...
  planSelect?.addEventListener('change', () => {
    updateSelectedPlan(planSelect.value);
    setStatus('', 'info');
  });
  ...
  form.addEventListener('submit', async (event) => {
    ...
    if (!form.reportValidity()) {
      setStatus('Fill in the required fields before sending the quote request.', 'error');
      return;
    }

    const payload = Object.fromEntries(new FormData(form).entries());
    payload.planLabel = LAWN_PLAN_LABELS[payload.planCode] || payload.planCode;
    payload.page = getPageKey();
    ...
    try {
      const response = await fetch('/api/lawn-quote', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(result.error || 'Quote request failed. Try again in a moment.');
      }
      ...
      form.reset();
      if (form.elements.city) form.elements.city.value = defaultCity;
      if (form.elements.province) form.elements.province.value = defaultProvince;
      updateSelectedPlan('');
      setStatus(result.message || 'Quote request sent. Green Wolf will follow up shortly.', 'info');
    } catch (error) {
      emitAnalyticsEvent('lawn_quote_submit_error', {
        page: getPageKey(),
        plan: payload.planCode,
        message: error.message
      });
      setStatus(error.message || 'Quote request failed. Try again in a moment.', 'error');
    } finally {
      submitButton.disabled = false;
      submitButton.textContent = 'Send to Green Wolf';
    }
  });
}
function init() {
